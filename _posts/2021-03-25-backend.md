...
def connect_with_database():
    ...
    dbcon = sqlcon.connect(
        host = "localhost",
        user = MySQL_username,			# MySQL_username and MySQL_password is fetched from the EasyGUI
        password = MySQL_password,		# dialog box	
    )
    cursor = dbcon.cursor()

    speak("Successfully established connection with Database")
    # speak is a function defined using pyttsx3 in the start of the code which converts Text-to-Speech
